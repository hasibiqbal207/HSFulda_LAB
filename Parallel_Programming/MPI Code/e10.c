...
int main(int argc, char** argv) {
	...
	if(rank==0){
		...
		for(int i=1; i < size; i++){
			...
			MPI_Recv(&randomNumber, 1, MPI_INT, MPI_ANY_SOURCE, 222, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		...
		MPI_Send(&rank, 1, MPI_INT, 0, 221, MPI_COMM_WORLD);
		usleep(10);
		MPI_Send(&randomNumber, 1, MPI_INT, 0, 222, MPI_COMM_WORLD);
